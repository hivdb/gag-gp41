...
if (distanceFormat != 1 && distanceFormat != 3)
{
	if (distanceFormat == 2)
	{
		...
		fprintf (outputFile, CLEAR_FILE, seqString);
		for (i = 1; i<ds.species; i=i+1)
		{
			GetString	(seqString,ds,i);
			fprintf (outputFile, "\t", seqString);
		}
	}
	else
	{
	...
	for (i = 0; i<ds.species; i=i+1)
	{
		...
		{
			fprintf (outputFile,"\t",distanceMatrix[i][j]);
		}
	}
}
else
{
	if (distanceFormat == 1)
	{
		fprintf (outputFile,CLEAR_FILE,distanceMatrix);
	}
	else
	{
		GetString (seqString,ds,-1);
		fprintf (outputFile,CLEAR_FILE,"Sequence1\tSequence2\tDistance");
		for (i = 0; i<ds.species; i=i+1)
		{
